---
domain: pymupdf.readthedocs.io
fetch_date: '2026-05-18T12:36:31.374051'
status: ok
...
To retrieve your document content in **Markdown** use the `to_markdown()`
...
To retrieve your document content in **JSON** use the `to_json()`
...
To retrieve your document content in **TXT** use the `to_text()`
...
By default PyMuPDF4LLM includes a layout analysis module to enhance output results. To disable this module you can do so by calling the `use_layout()`
...
**Garbled text** — if a page does contain text but too many characters are unreadable (e.g. `"�����"`

), OCR is applied **for the affected text areas only**, not the full page. This preserves already-readable text, images, and vectors while recovering only what is broken.
...
to bypass the auto-detection check entirely:
...
Forcing OCR on clean, text-based PDFs will slow down processing significantly and may reduce output quality. Only use `force_ocr=True`

when you have reason to distrust the native text layer.
...
Specify the language to be used by the Tesseract OCR engine. Default is `"eng"`

(English). Make sure that the respective language data files are installed. Remember to use correct Tesseract language codes. Multiple languages can be specified by concatenating the respective codes with a plus sign `"+"`
...
**Process only the pages you need**using the`pages`

parameter to avoid running OCR on the entire document.**Cache results**— write the output to disk after the first run so you don’t re-process the same file.**Use**`force_ocr=False`

(the default) so clean pages skip OCR entirely.**Resize images before passing to OCR**— very high DPI scans can slow Tesseract down without improving accuracy.
