...
fetch_date: '2026-05-18T12:38:06.677045'
status: ok
...
-
In supervised learning, we measured

*log loss*rather than employing sampling-based evals, with the same goal of generality in mind. Log loss measurement gives clean results and scaling laws over ranges of training steps and training parameters.
...
$$W' = W + \frac{\alpha}{r}BA$$Where $r$ is the LoRA rank, $\alpha$ is the LoRA scaling factor, and $A$, $B$ are the LoRA weight matrices (of rank $r$). We use $\alpha = 32$ for the experiments in this article, following standard practice from other implementations.
...
In our experiments, we used the standard parametrization used in the Huggingface `peft`

libraryPEFT: State-of-the-art Parameter-Efficient Fine-Tuning methods (Mangrulkar et al, 2022) proposed by Hu et al: a uniform distribution for $A$ with scale $1/\sqrt{d_{in}}$, zero initialization for $B$, the same LR for both, and $α = 32$. We were unable to improve on these hyperparameters in our experimentation.
...
$$\text{LR} = M_{\text{LoRA}} \cdot \left(\frac{2000}{\text{hidden size}}\right)^{\text{model pow} + \text{LoRA pow}}$$Where:
...
- $y = Wx$ ($N^2$ multiply–adds)

**Backward**

- $\bar{x} = W^T \bar{y}$ ($N^2$ multiply–adds)
- $\bar{W} \mathrel{+}= x \bar{y}^T$ ($N^2$ multiply–adds)
