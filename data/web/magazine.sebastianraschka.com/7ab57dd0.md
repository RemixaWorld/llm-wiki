...
fetch_date: '2026-05-18T12:36:37.591363'
status: ok
...
As you can see in the figure above, I included an additional * projector* module that follows the image encoder. This

*projector*is usually just another
...
**linear projection**![](https://substackcdn.com/image/fetch/$s_!TaTW!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F5d0be64c-da90-4193-86db-804f6a8a0abb_1542x1242.png)
...
In the figure above, x is the input, and *W q* is a weight matrix used to generate the queries (

*Q*). Similarly,

*K*stands for keys, and

*V*stands for values. A represents the attention scores matrix, and

*Z*are the inputs (x) transformed into the output context vectors. (If this seems confusing, you may find a comprehensive introduction in Chapter 3 of my Build a Large Language Model from Scratch book helpful; alternatively, you may also find my article, Understanding and Coding Self-Attention, Multi-Head Attention, Cross-Attention, and Causal-Attention in LLMs helpful here.)
...
In the case of the original transformer architecture in the *Attention Is All You Need* paper, the two inputs *x 1* and

*x*correspond to the sequence returned by the encoder module on the left (

2*x*) and the input sequence being processed by the decoder part on the right (

2*x*). In the context of a multimodal LLM,

1*x*is the output of an image encoder. (Note that the queries usually come from the decoder, and the keys and values typically come from the encoder.)

2Note that in cross-attention, the two input sequences *x 1* and

*x*can have different numbers of elements. However, their embedding dimensions must match. If we set

2*x*, this is equivalent to self-attention.

1= x2# 3. Unified decoder and cross-attention model training
