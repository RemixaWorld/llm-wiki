...
fetch_date: '2026-05-18T12:38:13.468038'
status: ok
...
*t*: position of the current query token;*s*: position of a previous token in the sequence (0 ≤ s < t);*j*: the index over the different indexer heads (Figure 10 above only showed one head for simplicity), so*q*means “query vector for current tokent, j*t*in indexer head*j*“.
