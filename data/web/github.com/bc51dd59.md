---
domain: github.com
fetch_date: '2026-05-18T12:37:22.434724'
status: ok
url: https://github.com/VikParuchuri/marker/tree/foundation-integration
---
...
flag to ensure the bad lines are fixed and formatted. You can also set`--force_ocr`

to force OCR on all lines, or the`strip_existing_ocr`

to keep all digital text, and strip out any existing OCR text.
...
will process pages 0, 5 through 10, and page 20.`--output_format [markdown|json|html]`
...
: Uses an LLM to improve accuracy. You will need to configure the LLM backend - see below.`--format_lines`

: Reformat all lines using a local OCR model (inline math, underlines, bold, etc.). This will give very good quality math output.`--force_ocr`
...
to keep individual characters and bounding boxes. You can also set `--force_ocr`
