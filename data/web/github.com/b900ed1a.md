---
domain: github.com
fetch_date: '2026-05-18T12:36:06.812541'
status: ok
...
![img.png](/jina-ai/late-chunking/raw/main/img/context-problem.png)


For example, if we split a Wikipedia article into sentence-length segments, as in the example above, a RAG system might not be able to answer a query like "What is the population of Berlin?" The city name and the population never appear together in a single segment, and lacking any larger document context. An LLM to which one of the segments is presented cannot resolve the anaphoric references like "it" or "the city".

To overcome this problem, we take advantage of the long input sequences that recent embedding models like `jina-embeddings-v2-base-en`

can process.
These models support much longer input texts, for example, 8192 tokens for `jina-embeddings-v2-base-en`
...
To reporoduce the evaluation, you can install the dependencies with `pip install .`
