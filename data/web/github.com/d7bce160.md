---
domain: github.com
fetch_date: '2026-05-18T12:36:06.479309'
status: ok
url: https://github.com/VikParuchuri/surya/tree/master?utm_source=ainews&utm_medium=email&utm_campaign=ainews-everybody-shipped-small-things-this
---
...
Our managed platform runs our latest open source model, Chandra — higher accuracy than Surya, with zero data retention by default, SOC 2 Type 2, and custom BAAs.
...
is the default, which will format text and give you bboxes. If you get bad performance, try`ocr_without_boxes`

, which will give you potentially better performance but no bboxes. For blocks like equations and paragraphs, try`block_without_boxes`
...
- the detected text and bounding boxes for each line`text`

- the text in the line`confidence`
...
- the individual characters in the line`text`

- the text of the character`bbox`
...
- the page number in the file`image_bbox`
...
- detected bounding boxes for text`bbox`
...
- vertical lines detected in the document`bbox`
...
- detected table rows`bbox`

- the bounding box of the table row`row_id`

- the id of the row`is_header`

- if it is a header row.

`cols`

- detected table columns`bbox`

- the bounding box of the table column`col_id`

- the id of the column`is_header`

- if it is a header column

`cells`

- detected table cells`bbox`
...
- the page number in the file`table_idx`
...
- installs main and dev dependencies`poetry shell`
