---
domain: github.com
fetch_date: '2026-05-18T12:34:56.415716'
status: ok
...
- [2026/4] OpenRLHF 0.10 adds
**Multi-Turn VLM RL**— multi-step interactions with images in both prompts and environment feedback (e.g. screenshots). Example: vlm_multiturn_agent.py - [2026/4] OpenRLHF 0.10 adds
**VLM (Vision-Language Model) RLHF support**— train VLMs like Qwen3.5 with image inputs end-to-end. Training script: train_vlm_math_hybrid_engine.sh - [2026/2] ProRL V2 uses REINFORCE++-baseline to train a state-of-the-art 1.5B reasoning model with prolonged RL training. Training script: train_prorlv2_math_hybrid_engine.sh
- [2026/10] ScaleRL validates the effectiveness of REINFORCE++-baseline in large-scale training scenarios. Releases REINFORCE++ slides
- [2026/6] Magistral uses the method quite similar to REINFORCE++-baseline to train the reasoning models.
- [2026/5] MARTI has been released as a fork of OpenRLHF. It is designed to train LLM-based multi-agent systems using RL, by integrating centralized multi-agent interactions with distributed policy training.
- [2026/5] OpenRLHF 0.8.0 supports async RLHF training via
`--train.async_enable`

and async agent RLHF via`--train.agent_func_path`

. See train_reinforce_baseline_ray_agent_async.sh for a runnable example. - [2026/4] Post the blog Accelerating RLHF with vLLM, Best Practice from OpenRLHF
- [2026/4] Clean OpenRLHF: Refactored the source code based on Single Controller and Unified Packing Samples
- [2026/3] The CMU Advanced Natural Language Processing Spring 2026 course uses OpenRLHF as the RLHF framework teaching case.
- [2026/2] Logic-RL and PRIME demonstrate that REINFORCE++ is more stable in training compared to GRPO and faster than PPO.
- [2026/2] LMM-R1 is a fork of OpenRLHF, aimed at providing high-performance RL infrastructure for reproduction of DeepSeek-R1 on multimodal tasks.
- [2026/2] MIT & Microsoft proposed the On the Emergence of Thinking in LLMs I: Searching for the Right Intuition using OpenRLHF
- [2026/1] HKUST reproduced the DeepSeek-R1-Zero and DeepSeek-R1 training on small models using OpenRLHF
...
|---|---|---|
Token-in-Token-out |
All sampling produces token-level trajectories | Zero text-level mismatch |
Unified Interface |
Same `AgentExecutorBase` API for all modes |
Switch modes with one flag |
Algorithm-Agnostic |
RL algorithms (PPO, REINFORCE++, etc.) are decoupled from agent executors | Any algorithm works with any mode |
Extensible |
Plug in custom rewards/environments easily | Rapid experimentation |
Production-Ready |
Sync/Async/Hybrid Engine support | From research to deployment |
...
|---|---|---|---|
Single-Turn |
Standard RLHF, custom reward functions | Optional `reward_func()` |
⭐ Default (99% use cases) |
Multi-Turn |
Multi-step reasoning, interactive environments | `reset()` + `step()` |
⭐⭐ Advanced |
...
|---|---|---|---|
PPO |
(default) | Full critic network | Stable training, proven results |
REINFORCE++ |
`reinforce` |
...
) - Hybrid Engine for maximum GPU utilization
...
for an agent executor that wraps vLLM as a local OpenAI Agent Server - Async pipeline (
`--train.async_enable`

) for higher throughput: train_reinforce_baseline_ray_agent_async.sh
...
|---|---|---|
SFT |
train_sft.sh | Supervised fine-tuning with packing |
DPO/IPO/cDPO |
train_dpo_llama.sh | Direct preference optimization |
Reward Model |
train_rm.sh | Train reward models |
...
) for all training modes - vLLM acceleration (
`--vllm.num_engines`

) for fast generation - DAPO dynamic filtering (
`--algo.dynamic_filtering_enable`

)- 🎲 Dynamic Sampling: for each prompt, generate multiple responses and
**filter**them by your reward / agent**0–1**signal`scores`
...
and either`--reward.remote_url`

or`--train.agent_func_path`
...
- DeepSpeed AutoTP for tensor parallelism (see
`--ds.tensor_parallel_size`

in training scripts) - RingAttention for long context (
`--ds.ring_attn_size`
...
- VLM (Vision-Language Models) — single-turn and multi-turn with image feedback (
`--data.image_key`
...
- AdamW (default):
`--{actor,critic}.optim adam --{actor,critic}.adam.lr 2e-6`

- Muon (via DeepSpeed ≥ 0.18.2, 2D weights only; embeddings / head / 1-D params use aux-AdamW):
`--{actor,critic}.optim muon --{actor,critic}.muon.lr 1e-4 --{actor,critic}.muon.momentum 0.95`

. Newton-Schulz produces scale-invariant updates, so disable global grad clipping with`--{actor,critic}.max_norm 0`

(the Adam default`1.0`

would clip away the Muon update).

**Reward Shaping**

- DAPO-style overlong penalty for length control (
`--reward.overlong_buffer_len`

,`--reward.overlong_penalty_factor`

) — soft-penalize responses that exceed`max_new_tokens - overlong_buffer_len`

- ProRL-style truncation penalty (
`--reward.stop_properly_penalty_coef`

) — for samples with`finish_reason='length'`

:`coef ∈ [0, 1]`

multiplicatively scales the reward;`coef < 0`

sets the reward to that fixed value (e.g.`-0.5`
...
) and TensorBoard (`--logger.tensorboard_dir`
...
) - Best-checkpoint saving on eval metrics (
...
) — supported in async training - Multi-process data loading (
`--data.dataloader_num_workers`

, available for PPO/SFT/RM/DPO) - PPO observability: actor/critic grad-norm and per-phase timing (
...
We recommend **vLLM 0.19.1+** for best performance. See Dockerfiles and Nvidia-Docker Install Script.
...
: Specify JSON key name for input data`--data.apply_chat_template`

: Use HuggingFace tokenizer's chat template`--data.input_template`

: Custom template string (alternative to chat template)`--data.prompt_probs`

/`--data.dataset_probs`

: Mix multiple datasets (e.g.,`0.1,0.4,0.5`

)`--eval.dataset`

: Specify evaluation dataset path
...
OpenRLHF's model checkpoint is fully compatible with HuggingFace models. You can specify the model name or path using `--actor.model_name_or_path {name or path}`

, `--reward.model_name_or_path {name or path}`

and `--critic.model_name_or_path {name or path}`
...
**For reasoning tasks (RLVR)**: Use `--algo.advantage.estimator reinforce_baseline`

for REINFORCE++-baseline—it's robust to different reward scales.
...
**Troubleshooting GPU index errors**: Set `export RAY_EXPERIMENTAL_NOSET_CUDA_VISIBLE_DEVICES=1`

if you encounter DeepSpeed GPU device setup issues.
...
**Key Parameter**: `--data.label_key answer`

passes the "answer" field from your dataset to `reward_func`
...
(larger = more off-policy, default 1) - Partial rollout:
`--train.partial_rollout_enable`

— uses vLLM pause/resume for weight sync instead of locking, allowing generation to overlap with training. In-flight samples may contain tokens from both old and new weights.

**Training Modes**:

**Synchronous**: Default, better stability**Asynchronous**: Higher throughput, may affect convergence**Hybrid Engine**: Best GPU utilization with`--train.colocate_all`

(remove`--train.async_enable`

)

Note

For fully custom token-level execution, inherit `AgentExecutorBase`

and implement `execute()`

. This design enforces the **token-in-token-out principle** to keep sampling and training consistent.
...
- Single-turn: train_ppo_ray_hybrid_engine.sh
- Custom reward: train_ppo_with_reward_fn.sh
- Multi-turn: train_reinforce_baseline_ray_agent_async.sh
- Multi-turn VLM (image feedback): vlm_multiturn_agent.py

For multi-turn agents that need an OpenAI-compatible chat API (e.g., integrating external tool-use frameworks), `agent_func_openai_server_executor.py`

wraps vLLM as a local `/v1/chat/completions`

server while collecting token-level traces for RL training.
...
to plug in your own multi-turn workflow
...
Most stable — strictly on-policy, every rollout uses the latest weights. Serial generate→train cycle. |
Research, sensitive RL algorithms, reproducibility, recipe validation |
Async Training |
`--train.async_enable` `--train.async_queue_size N` |
...
Maximum overlap — vLLM pause/resume instead of locking, in-flight samples may mix old/new weights. Most aggressive off-policy. |
Pushing async throughput further; pair with `--algo.advantage.is_correction_enable` |
...
Prefix Caching |
vLLM config | `n_samples_per_prompt` > 1 |
Oversampling |
...
options - ✅ Reduce batch sizes
- ✅ Enable gradient checkpointing

**Generation Phase**: Maximize`--rollout.micro_batch_size`

, minimize vLLM TP size**Training Phase**: Maximize`--train.micro_batch_size`

, enable`--ds.packing_samples`

**vLLM**: Always use`--vllm.sync_backend nccl`


Tip

**Quick Start Template**: For 8x A100 (80GB), try Hybrid Engine + `--vllm.gpu_memory_utilization 0.5`
