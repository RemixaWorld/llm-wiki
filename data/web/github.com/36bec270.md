---
domain: github.com
fetch_date: '2026-05-18T12:34:56.497275'
status: ok
...
**Test Runner:**Executes the command or scripts to run the test suite and generate code coverage reports.**Coverage Parser:**Validates that code coverage increases as tests are added, ensuring that new tests contribute to the overall test effectiveness.**Prompt Builder:**Gathers necessary data from the codebase and constructs the prompt to be passed to the Large Language Model (LLM).**AI Caller:**Interacts with the LLM to generate tests based on the prompt provided.
...
If you run the same command without the `--record-mode`
...
You may also record LLM responses from a separate test run. Run a test as you normally would, and add the `--record-mode`
...
The table below explains the behavior of the test runner depending on whether the `--record-mode`

flag is set and whether a recorded file already exists:
...
Recorded responses are stored in the `stored_responses`

folder. Files are named based on the test name and a hash value that depends on the contents of the source and test files.
...
You can suppress logs using the `--suppress-log-files`

flag. This prevents the creation of the `run.log`

, `test_results.html`

, and the test results `db`
...
- Instructions for Adding Another Coverage Type to the
`CoverageProcessor`

Class - Using a Test Database with Cover Agent
