---
domain: github.com
fetch_date: '2026-05-18T12:36:35.841079'
status: ok
...
Tabled is now deprecated. The functionality here has been migrated to marker.
...
Read more, and see other CLI options here.
...
- bbox of the cell within the table bbox`text`

- the text of the cell`row_ids`

- ids of rows the cell belongs to`col_ids`

- ids of columns the cell belongs to`order`

- order of this cell within its assigned row/column cell. (sort by row, then column, then order)

`rows`

- bboxes of the detected rows`bbox`

- bbox of the row in (x1, x2, y1, y2) format`row_id`

- unique id of the row

`cols`

- bboxes of detected columns`bbox`

- bbox of the column in (x1, x2, y1, y2) format`col_id`
...
- page number within the document`tnum`
