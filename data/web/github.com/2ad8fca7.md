---
domain: github.com
fetch_date: '2026-05-18T12:34:49.101524'
status: ok
...
: Schedule-free variants of SGD`AdamWScheduleFree`

and`AdamWScheduleFreeReference`

: Schedule-free variants of AdamW`RAdamScheduleFree`

: Schedule-free variant of RAdam, which eliminates the need for both learning rate scheduling and warmup (implementation community contributed)- Experimental
`ScheduleFreeWrapper`

to combine with other optimizers

`ScheduleFreeReference`
...
$\beta$ than you may expect from standard momentum. Our default of$0.9$ works on most problems but it may be necessary to increase the value to$0.95$ or$0.98$ particularly for very long training runs. - There is no need to use a learning rate scheduler, however the code is compatible with one.
...
We offer a highly experimental wrapper version `ScheduleFreeWrapper`
...
If you set weight decay on the base optimizer, it computes weight decay at `weight_decay_at_y`
