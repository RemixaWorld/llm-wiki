---
domain: github.com
fetch_date: '2026-05-18T12:37:48.688938'
status: ok
url: https://github.com/peremartra/optipfair
---

Companion Library:OptiPFair is the official open-source implementation for the upcoming Manning bookRearchitecting LLMs. Explore the theory, mechanics, and advanced research behind these algorithms in the book's repository.
...
is still accepted and maps to PPM.
...
trains the pruned model to recover performance using a combined loss (cross-entropy + Skew KLD). No custom training loop required.

| Parameter | Default | Description |
|---|---|---|
`alpha` |
`0.6` |
Weight of task loss (cross-entropy) |
`beta` |
`0.40` |
Weight of logits loss (Skew KLD) |
`gamma` / `delta` |
`0.0` |
Feature alignment — set to 0 for labels-only distillation |
`temperature` |
`2` |
Softens teacher distribution for transfer |
`scheduler` |
`"cosine"` |
...
`return_stats` |
`False` |
Returns `loss_history` dict for plotting |


Tip:Use`optipfair_llm_reference_manual.txt`

with any LLM assistant (ChatGPT, Claude) for guided help tuning these parameters.
