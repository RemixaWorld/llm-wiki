---
domain: blog.mlc.ai
fetch_date: '2026-05-18T12:35:43.093993'
status: ok
...
![](/img/mlc-engine/rest-server.gif)
...
![](/img/mlc-engine/web-llm.png)


### Discussions

One thing we might realize is that these APIs follow the exact same style `engine.chat.completions.create`

so we can have the same development experience across these platforms. We also make use of language local features as much as possible to create structured inputs and outputs and enable async streaming so engine activities do not block the UI.
...
MLCEngine supports two levels of JSON mode: general JSON response and JSON schema customization. The general JSON mode constrains the response to conform to JSON grammar. To use the general JSON mode, pass argument `response_format={"type": "json_object"}`

to chat completion. Below is a request example with JSON mode:

![](/img/mlc-engine/json-general.png)
...
![](/img/mlc-engine/json-schema.png)
...
![](/img/mlc-engine/multigpu-cuda.png)


![](/img/mlc-engine/multigpu-rocm.png)
