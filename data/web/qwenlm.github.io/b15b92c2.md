...
fetch_date: '2026-05-18T12:34:46.277355'
status: ok
...
To utilize the Qwen1.5-MoE model with the `qwen2_moe`

implementation in Hugging Face’s transformers, since the latest release does not include this feature yet, you will have to install transformers from source instead of installing it via pip or conda:
...
The following step is indeed straightforward and akin to using models such as Qwen1.5, Mistral, or Llama. We demonstrate an example of the usage of Qwen1.5-MoE-A2.7B-Chat. To use the quantized model instead, you can just substitute the model name `Qwen1.5-MoE-A2.7B-Chat`

with `Qwen1.5-MoE-A2.7B-Chat-GPTQ-Int4`

(temporarily AWQ is not supported).
