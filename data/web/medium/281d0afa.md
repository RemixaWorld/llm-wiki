...
fetch_date: '2026-05-18T12:50:58.564034'
status: ok
url: https://ai.gopubby.com/mcp-by-anthropic-will-blow-your-mind-2b39c76c0a0b
---
...
[ ![Manpreet Singh](https://miro.medium.com/v2/resize:fill:64:64/1*YaViKfuc2uAV4OhpLOsZXw.jpeg) ](<https://medium.com/@singh.manpreet171900?source=post_page---byline--2b39c76c0a0b--------------------------------------->)

[Manpreet Singh](<https://medium.com/@singh.manpreet171900?source=post_page---byline--2b39c76c0a0b--------------------------------------->)
...
> Trust me this is not a clickbait, MCP will r**evolutionize AI Interactions**
...
That’s where Anthropic’s **_Model Context Protocol (MCP)_ **comes in.
...
MCP works like a universal connector by allowing LLMs to connect to _live data sources_.
...
The Model Context Protocol changes all that by giving everyone a _universal, open standard_ for connecting AI assistants to any tool or data source.
...
```json { "mcpServers": { "git": { "command": "uvx", "args": ["mcp-server-git", "--repository", "path/to/git/repo"] } }} ```
...
Let’s say you’re doing research. With MCP and a search API like Brave Search and your AI assistant could _search live_ for new articles and pick out the important parts, and summarize them — all without you having to type anything into a browser.
...
```json { "mcpServers": { "braveSearch": { "command": "npx", "args": ["-y", "@modelcontextprotocol/server-brave"] } }} ```
...
```json { "mcpServers": { "googleDrive": { "command": "npx", "args": ["-y", "@modelcontextprotocol/server-googledrive"] } }} ```
...
```json { "mcpServers": { "filesystem": { "command": "npx", "args": ["-y", "@modelcontextprotocol/server-filesystem", "/path/to/allowed/files"] } }} ```
...
```python from mcp_server import MCPServerclass MyCustomServer(MCPServer): def handle_request(self, request): if request['type'] == 'get_data': return {'data': 'Here is your custom data!'}if __name__ == "__main__": server = MyCustomServer() server.run() ```
...
The AI can _send_ data too — it can read from GitHub and also make changes.
...
This could become the _standard_ for making AI models more useful.
...
**_Want to read more articles on claude do check these also._**

## [Now Use Top LLMs for free as Coding Assistant in VScode: Copilot ArenaHey there!ai.gopubby.com](</now-use-top-llms-for-free-as-coding-assistant-in-vscode-copilot-arena-1e556fc683ae?source=post_page-----2b39c76c0a0b--------------------------------------->)

## [Lets Decode Complex PDFs Using Claude’s PDF ToolCreated by authorai.gopubby.com](</lets-decode-complex-pdfs-using-claudes-pdf-tool-90332eaa3b78?source=post_page-----2b39c76c0a0b--------------------------------------->)

## [How to Use Claude 3.5 for Data Analysis(Secret tool)Most of us dont know about this featureai.gopubby.com](</how-to-use-claude-3-5-for-data-analysis-secret-tool-d4316883df64?source=post_page-----2b39c76c0a0b--------------------------------------->)
