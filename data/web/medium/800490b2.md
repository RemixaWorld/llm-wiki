---
domain: medium.com
fetch_date: '2026-05-18T12:46:12.662624'
status: ok
url: https://medium.com/@infiniflowai/dense-vector-sparse-vector-full-text-search-tensor-reranker-best-retrieval-for-rag-9c86b02a55ef
---

# Dense vector + Sparse vector + Full text search + Tensor reranker = Best retrieval for RAG?

[ ![InfiniFlow](https://miro.medium.com/v2/resize:fill:64:64/1*VIOVinD7tDsinrVyUvlV9w.png) ](</@infiniflowai?source=post_page---byline--9c86b02a55ef--------------------------------------->)

[InfiniFlow](</@infiniflowai?source=post_page---byline--9c86b02a55ef--------------------------------------->)
...
![image](https://miro.medium.com/v2/resize:fit:700/1*BrHAxxZhxJwKoNP9UFfmhA.png)
...
``` [0.2, 0.3, 0.5, 0.7,……][{331: 0.5}, {14136: 0.7}] ```
...
![image](https://miro.medium.com/v2/resize:fit:700/1*4z4prymREaXLxAqroAmKlA.png)
...
![image](https://miro.medium.com/v2/resize:fit:700/1*UY6bYWXEfNDZKTjK-A0Y8g.png)
...
```json res = table_obj.output(['*']).match_vector('vec', [3.0, 2.8, 2.7, 3.1], 'float', 'ip', 1).match_sparse('sparse_vec', {"indices": [0, 10, 20], "values": [0.1, 0.2, 0.3]}, 'float', 'ip', 1).match_text('title, body', 'hello world', 'topn=10').fusion('rrf').to_pl() ```
...
```json res = table_obj.output(['*']).match_vector('vec', [3.0, 2.8, 2.7, 3.1], 'float', 'ip', 1).match_sparse('sparse_vec', {"indices": [0, 10, 20], "values": [0.1, 0.2, 0.3]}, 'float', 'ip', 1).fusion('match_tensor','column_name=t;search_tensor=[[0.0, -10.0, 0.0, 0.7], [9.2, 45.6, -55.8, 3.5]];tensor_data_type=float;match_method=MaxSim;topn=2').match_text('title, body', 'hello world', 'topn=10').fusion('weighted_sum', 'weights=0.8, 0.2').to_pl() ```
...
ColBERT is introduced as a ranking model in “ _Colbert: Efficient and effective passage search via contextualized late interaction over_ _bert_ , _SIGIR_ _2020_ ” (reference [4]), which is one of the most cited papers over the past four years in the field of information retrieval. According to this paper, mainstream ranking model architectures follow these three paradigms:
...
![image](https://miro.medium.com/v2/resize:fit:700/1*fCHXqhpikH1eTA6O_3bFRg.png)
...
![image](https://miro.medium.com/v2/resize:fit:700/1*sxqMmmHdwhzVxKUiYiUuCg.png)
...
![image](https://miro.medium.com/v2/resize:fit:700/1*J90k7OJTCDJyfxcBe6bNZA.png)
...
![image](https://miro.medium.com/v2/resize:fit:700/1*aew1b9ZDr0TiiO4RDOwORg.png)
...
![image](https://miro.medium.com/v2/resize:fit:700/1*kIjoGw69ZGjcpsFMj_CEnw.png) ``` res = table_obj.output(['*']).match_tensor('t',[[0.0, -10.0, 0.0, 0.7], [9.2, 45.6, -55.8, 3.5]], 'float', 'maxsim').match_text('title, body', 'hello world', 'topn=10').fusion('weighted_sum', 'weights=0.8, 0.2').to_pl() ```

Secondly, Infinity introduces the Tensor Array data type to handle long documents that exceed the token limits:

![image](https://miro.medium.com/v2/resize:fit:700/1*4GRpw6H9zjVsTW7z9LIbNg.png)
...
![image](https://miro.medium.com/v2/resize:fit:700/1*_ixrCL0PqtrfowSM_WmkSw.png)
...
![image](https://miro.medium.com/v2/resize:fit:700/1*1hpi0umWoieT5fiOu3cfMw.png)
