...
fetch_date: '2026-05-18T12:45:54.357202'
status: ok
url: https://levelup.gitconnected.com/hosting-multiple-web-apps-and-apis-with-nginx-reverse-proxies-29f674965297
---
...
[ ![Irtiza Hafiz](https://miro.medium.com/v2/da:true/resize:fill:64:64/0*zyHb8yqCDszgfGj3) ](<https://irtizahafiz.medium.com/?source=post_page---byline--29f674965297--------------------------------------->)

[Irtiza Hafiz](<https://irtizahafiz.medium.com/?source=post_page---byline--29f674965297--------------------------------------->)
...
The first step is to connect my root domain ([irtizahafiz.com](<http://irtizahafiz.com>)) to my server.
...
Once that’s done, whenever a user types [irtizahafiz.com](<http://irtizahafiz.com>) in their browser, the request will make its way to your server’s port 80 or port 443.
...
``` server { listen 443 ssl; server_name irtizahafiz.com; location / { proxy_pass http://127.0.0.1:3000; } ssl_certificate /etc/letsencrypt/live/irtizahafiz.com/fullchain.pem; ssl_certificate_key /etc/letsencrypt/live/irtizahafiz.com/privkey.pem; include /etc/letsencrypt/options-ssl-nginx.conf; ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem;}server { listen 80; server_name irtizahafiz.com; # Redirect HTTP requests to HTTPS if ($scheme != "https") { return 301 https://$host$request_uri; }} ```
...
Once that’s done when someone types [umami.irtizahafiz.com](<http://umami.irtizahafiz.com>) in their browser, the request will make its way to your server’s port 80 or port 443 (depending on HTTP or HTTPS).
...
``` server { listen 443 ssl; server_name umami.irtizahafiz.com; location / { proxy_pass http://127.0.0.1:7001; proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for; proxy_set_header Host $host; } ssl_certificate /etc/letsencrypt/live/irtizahafiz.com/fullchain.pem; ssl_certificate_key /etc/letsencrypt/live/irtizahafiz.com/privkey.pem; include /etc/letsencrypt/options-ssl-nginx.conf; ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem;}server { listen 80; server_name umami.irtizahafiz.com; # Redirect HTTP requests to HTTPS if ($scheme != "https") { return 301 https://$host$request_uri; }} ```

Again, here’s how the Nginx reverse proxy is working for a subdomain:

* For HTTPS traffic to [umami.irtizahafiz.com](<http://umami.irtizahafiz.com>), the request is proxy passed (or forwarded) to port 7001 where the Umami docker instance is running.
